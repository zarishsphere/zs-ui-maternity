# TECH-DESIGN-MVP — `zs-ui-maternity`
...
This document defines the **technical architecture, implementation design, complete repository tree, and acceptance criteria** for the MVP of `zs-ui-maternity`.
...
```typescript
// hooks/useMaternityData.ts
import { useFHIRQuery } from '@zarishsphere/zs-pkg-ui-fhir-hooks'

export function useMaternityData(tenantId: string) {
  return useFHIRQuery({
...
const localRecords = await db.table('maternity').toArray()
...
```
zs-ui-maternity/src/components/
│   │   ├── MaternityMain/
│   │   ├── │   │   │   └── MaternityMain.tsx
